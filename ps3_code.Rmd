...
Model #2:
$log(wage) = \beta0 + exper\cdot\beta1 + tenure\cdot\beta2 + married\cdot\beta3 + south\cdot\beta4 + urban\cdot\beta5 + black\cdot\beta6 + educ\cdot\beta7 + \epsilon $

take the exponent of both side, which gives us:

$wage = e^{βo+exper\cdot\beta1+tenure\cdot\beta2+married\cdot\beta3+south\cdot\beta4+urban\cdot\beta5+black\cdot\beta6+educ\cdot\beta7+\epsilon}$

Ceteris Peribos (ie, letting all the other variables be zero $\gamma$ we get:
$wage = e^{\beta0+exper\cdot\beta1+Z\gamma +\epsilon}$

Taking the expectation of both sides conditional on the covariates:

$E[wage|X] = E[e^{\beta_0+married\cdot\beta_3+Zγ +\epsilon|X] = E[e\beta_0+married\cdot\beta_3+Z\gamma e\epsilon|X] $
And given that both $\epsilon$ is independent of X and the covariates are independent of each-other we get:
θ1 = eβo+marriedβ3+Zγ eε (5)
So there can only be two states of married, you either aren’t or you are.... 1 or 0, giving you one or the other extreme:
θ1−1 = eβo+β3+Zγ eε (6) θ1−0 = eβo+Zγ eε (7)
If you take the difference of equations 6 and 7 and then divide it by 6 you get the percentage difference which results in our consistent estimator θ1.
θˆ = 100[(eβo+β3+Zγ eε − eβo+Zγ eε)/(eβo+Zγ eε)] = 100[eβ3 − 1] (8) 1
