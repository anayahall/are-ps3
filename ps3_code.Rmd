...
In this case, save residuals from first 350 observations (1:350) and last 350 observations (585:935)
...
test_df <- data.frame(
  # Save residuals from regession and X var
  resid = as.vector(model_1$resid),
  tenure = wage_df$tenure) %>%
  # arrange by X (tenure)
  arrange(.,tenure)

# split resid into 2 groups (1:350) & (585:935)
e1 <- test_df$resid[1:350]
e2 <- test_df$resid[585:935]

# Run GQ Test
GQ_test(e1, e2, k=7)

```
According to the Goldfeld-Quandt Test, yes, we have a significant heteroskedasticity problem.
