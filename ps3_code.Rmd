...
white_test <- function(resid, cov_mat){
  ...
  int_mat <- matrix(NA, nrow = cov_k, ncol = cov_n)
  
  ## NEED TO MAKE THIS WORK!!!!!!!!!!!! Try combn function ??
  ...
  cov_mat %<>% rbind(.,int_mat)
  
  dep_var <- resid^2

  # y-hat for residual regression = X*beta
  dep_hat <- cov_mat %*% solve(t(cov_mat) %*% cov_mat) %*% t(cov_mat) %*% y_mat

  # Calculate R2
  SSM <- sum((dep_hat - mean(dep_var))^2)
  SST <- sum((dep_var - mean(dep_var))^2)

  # Calculate White test statistic = R^2 * n
  test_stat <- SSM/SST * nrow(cov_mat)

  # Calculate pvalue
  p-value <- 1 - pchisq(q = test_stat, df = ncol(cov_mat))
  ...
  # # white test results
  # whitetest_df <- data.frame(test_stat, p_value, df)
  # 
  # 
